...
| Accessibility/performance gates | Playwright smoke suite covers desktop/mobile core surfaces, synthetic onboarding, booking submissions, manual password reset, first-login MFA enrollment, website draft publishing, clean private-document signed downloads, platform retention approval/assignment, follow-up/notification operations, and privacy workflow controls; axe checks all implemented public/staff/platform routes; `infra/performance/load_profile.py` provides synthetic 20 req/s, 50-session profiles with exact short-lived cookie rotation and p95 thresholds; local execution passes all 82 browser tests | Partial; staging load execution and manual WCAG evidence remain open |
