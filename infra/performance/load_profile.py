...
def request_once(base_url: str, spec: RequestSpec, timeout: float, common_headers: dict[str, str]) -> Sample:
    ...
    def submit_at(executor: ThreadPoolExecutor) -> None:
        with lock:
            if time.monotonic() >= deadline:
                return
        future = executor.submit(request_once, args.base_url, choose_request(plan), args.timeout, common_headers)
        with lock:
